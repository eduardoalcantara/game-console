...
|---|---|---|---|---|---|
| ES-DE Android (pago) | **Manual** | Frontend | `https://es-de.org/` → Patreon / Galaxy Store / AppGallery | `resources/android/apk/` | `es-de-<versao>.apk` |
...
1. Comprar/baixar **ES-DE Android** e salvar em `resources/android/apk/es-de-<versao>.apk`.
2. Instalar **DuckStation** pela Play Store no celular/tablet **ou** fornecer APK local se tiver.
3. Colocar **BIOS PS1** (e outras BIOS se necessario) em `resources/android/bios/`.
4. Garantir **platform-tools** (`adb`) no PC.
5. Preencher hashes do ES-DE / BIOS / DuckStation (se APK) na tabela.
...
|---|---|---|---|---|---|
| `apk/es-de-....apk` | Manual | _(pendente operador)_ | | | |
...
| `bios/...` (PS1) | Manual | _(pendente operador)_ | | | dump proprio |
| `bios/...` (Neo Geo) | Manual | _(pendente se usado)_ | | | dump proprio |

## Proximo passo

1. Operador: ES-DE + BIOS PS1 + DuckStation (Play Store ou APK proprio).
2. Seguir `setup-adb.md` (RetroArch ja esta em `resources/android/apk/`).
